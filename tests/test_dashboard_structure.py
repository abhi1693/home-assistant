...
class DashboardStructureTests(unittest.TestCase):
    ...
    def test_camera_backend_is_bounded_private_and_presence_aware(self):
        ...
        self.assertIn("web.FileResponse(path, headers=VIDEO_RESPONSE_HEADERS)", sensor)
        media = (ROOT / "custom_components/family_camera_events/media.py").read_text()
        self.assertIn('"Accept-Ranges": "bytes"', media)
        self.assertIn('"Cache-Control": "private, no-store"', media)
        self.assertIn("VIDEO_CACHE_MAX_CLIPS = 12", media)
        self.assertIn("VIDEO_CACHE_MAX_BYTES = 256 * 1024 * 1024", media)
        self.assertIn("output_file=partial", media)
