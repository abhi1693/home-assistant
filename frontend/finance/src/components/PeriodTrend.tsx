...
export default function PeriodTrend({period,comparison,metrics}:{period:ReportPeriod;comparison?:ReportPeriod;metrics:Metric[]}) {
  ...
  const rows=dates.map((month,index)=>{
    ...
    return {month,label:new Date(`${month}-01T12:00:00Z`).toLocaleDateString("en-IN",{month:"short",timeZone:"UTC"}),
  ...
  return <section className="period-trend" aria-label={`${metric.label} by month`}>
    ...
    <div className="period-trend-scroll"><div style={{minWidth:Math.max(290,rows.length*44)}}>
    <ResponsiveContainer width="100%" height={240}><BarChart data={rows} margin={{top:12,right:8,bottom:0,left:0}} accessibilityLayer>
      <CartesianGrid stroke="var(--nb-border)" strokeDasharray="3 3" vertical={false}/>
      <XAxis dataKey="label" tick={{fill:"var(--nb-muted)",fontSize:11}} interval={0}/>
      <YAxis tickFormatter={moneyCompact} width={72} tick={{fill:"var(--nb-muted)",fontSize:11}}/>
      <Tooltip contentStyle={{background:"var(--nb-panel-2)",border:"1px solid var(--nb-border)",borderRadius:8,fontSize:12}}
    ...
    {rows.length>6&&<p className="period-scroll-hint">Swipe the chart to see all months.</p>}
  </section>;
}
