...
import { MonthlyTotal, ReportPeriod, dateLabel, isMonthComparison } from "../lib/reportingPeriod";
import { DailyTotal, dailyComparisonRows } from "../lib/dailySpending";
...
type Metric={key:string;label:string;monthly?:MonthlyTotal[];comparison?:MonthlyTotal[];planned?:MonthlyTotal[];
  daily?:DailyTotal[];comparisonDaily?:DailyTotal[]};
export default function PeriodTrend({period,comparison,metrics}:{period:ReportPeriod;comparison?:ReportPeriod;metrics:Metric[]}) {
  ...
  const daily=isMonthComparison(period,comparison)&&metric.daily!==undefined;
  const monthlyRows=dates.map((month,index)=>{
  ...
  const rows=daily?dailyComparisonRows(metric.daily!,period,metric.comparisonDaily??[],comparison!):monthlyRows;
  // Keep the authoritative whole-period totals above either chart resolution.
  const total=monthlyRows.reduce((sum,r)=>sum+(r.current??0),0),previous=monthlyRows.reduce((sum,r)=>sum+(r.previous??0),0);
  const hasPlanned=rows.some(r=>(r.planned??0)>0);
  const title=`${metric.label} by ${daily?"day":"month"}`;
  return <section className="period-trend" aria-label={title}>
    <div className="period-trend-head"><div><h3>{title}</h3>
    ...
    <div className="period-trend-plot" {...interaction.plot}>
    <ResponsiveContainer width="100%" height="100%" minWidth={0}><BarChart data={rows} barGap="10%" barCategoryGap="18%" margin={{top:12,right:8,bottom:0,left:0}} accessibilityLayer>
      ...
      <Tooltip key={interaction.key} {...interaction.tooltip} wrapperStyle={{maxWidth:"calc(100% - 84px)"}} itemStyle={{whiteSpace:"normal"}} contentStyle={{whiteSpace:"normal",background:"var(--nb-panel-2)",border:"1px solid var(--nb-border)",borderRadius:8,fontSize:12}}
        labelFormatter={(_,payload)=>{const row=payload?.[0]?.payload;if(!row)return "";
          return daily?[row.current!=null?dateLabel(row.date):null,row.previous!=null?dateLabel(row.refDate):null].filter(Boolean).join(" · vs ")
            :`${monthLabel(row.month)}${comparison&&row.previous!=null?` · vs ${monthLabel(row.refMonth)}`:""}`;}}
