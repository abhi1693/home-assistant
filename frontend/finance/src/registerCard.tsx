...
export function defineCard(def: CardDef): void {
  class NetwrthCard extends HTMLElement {
    ...
    set hass(h: Hass) {
      const changed = !this._hass || this._hass.connection !== h.connection || this._hass.user?.id !== h.user?.id;
      this._hass = h;
    ...
    private _render() {
      ...
      if (!this._mount) {
        ...
        // Sibling of the card, outside its stacking context: chart
      ...
      if (this._config.allowed_user_id && this._hass.user?.id !== this._config.allowed_user_id) {
        this._root!.render(<div className="card"><div className="status">This dashboard is private.</div></div>);
        return;
      }
      ...
      this._root!.render(
        <OverlayContext.Provider value={this._overlay ?? null}>
          <Component key={`${hass.user?.id}:${JSON.stringify(this._config)}`} hass={hass} config={this._config} />
